-- Function deletes invalid shifts in Assigned_Shift upon update to Shift_Time
...
-- Run remove_shifts_upon_day_update function upon update to Shift_Time for each date affected
CREATE TRIGGER update_shifts_upon_day_update AFTER UPDATE OF start_time, end_time ON Shift_Time FOR EACH ROW
...
CREATE TRIGGER validate_shift_length_change BEFORE INSERT ON Assigned_Shift FOR EACH ROW
...
CREATE OR REPLACE FUNCTION valid_shift_timeframe()
  ...
  BEGIN
    IF
    (SELECT * FROM Shift_Time AS S
      WHERE NEW.date = S.date AND NEW.start_time < S.start_time OR NEW.end_time > S.end_time
    ) IS NOT NULL THEN
      RETURN NEW;
    ELSE
      RETURN NULL;
...
-- Ensures assigned shifts are only created within valid times in their corresponding Shift_Time.
